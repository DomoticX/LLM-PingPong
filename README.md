...
- **"Let LLM know max tokens"** — a per-agent checkbox. When enabled, the agent's system prompt automatically gets `Formulate your answer in max [maxTokens] tokens.` appended before each request, so the model knows the limit it's being cut off at (not all servers/models otherwise respect or expose `max_tokens` to the model itself).
